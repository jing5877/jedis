...
public class Jedis extends BinaryJedis implements JedisCommands, MultiKeyCommands, AdvancedJedisCommands,
		ScriptingCommands, BasicCommands, ClusterCommands, SentinelCommands, ModuleCommands {
	...
	public Jedis(final String host, final int port, final boolean ssl, final SSLSocketFactory sslSocketFactory,
	...
	/**
	 * Works same as <tt>ping()</tt> but returns argument message instead of
	 * <tt>PONG</tt>.
	 * 
	 * @param message
	 * @return message
	 ...
	 *            Only set the key if it already exist. EX|PX, expire time units: EX
	 *            = seconds; PX = milliseconds
	 * @return Status code reply
	 */
	@Override
	...
	/**
	 * Test if the specified keys exist. The command returns the number of keys
	 * exist. Time complexity: O(N)
	 * 
	 * @param keys
	 * @return Integer reply, specifically: an integer greater than 0 if one or more
	 *         keys exist, 0 if none of the specified keys exist.
	 */
	@Override
	 ...
	 * asynchronously.
	 * <p>
	 * Time complexity: O(1) for each key removed regardless of its size. Then the
	 * command does O(N) work in a different thread in order to reclaim memory,
	...
	/**
	 * EXPIREAT works exctly like {@link #expire(String, int) EXPIRE} but instead to
	 ...
	 * exist. Time complexity: O(N) where N is the number of keys that will be
	 * touched.
	 * 
	 * @param keys
	...
	/**
	 * Set the the respective keys to the respective values. {@link #mset(String...)
	...
	/**
	 * IDECRBY work just like {@link #decr(String) INCR} but instead to decrement by
	 ...
	 * <p>
	 * Note: this is actually a string operation, that is, in Redis there are not
	 * "double" types. Simply the string stored at the key is parsed as a base
	 * double precision floating point value, incremented, and then converted back
	 * as a string. There is no DECRYBYFLOAT but providing a negative value will
	 * work as expected.
	 * <p>
	 * Time complexity: O(1)
	 * 
	...
	/**
	 * If the key already exists and is a string, this command appends the provided
	 * value at the end of the string. If the key does not exist it is created and
	 * set as an empty string, so APPEND will be very similar to SET in this special
	 * case.
	 * <p>
	 ...
	 * @param key
	 * @param value
	 * @return Integer reply, specifically the total length of the string after the
	 *         append operation.
	 */
	@Override
	...
	/**
	 * Return a subset of the string from offset start to offset end (both offsets
	 * are inclusive). Negative offsets can be used in order to provide an offset
	 * starting from the end of the string. So -1 means the last char, -2 the
	 * penultimate and so forth.
	 * <p>
	 * The function handles out of range requests without raising an error, but just
	 * limiting the resulting range to the actual length of the string.
	 * <p>
	 * Time complexity: O(start+n) (with start being the start index and n the total
	 ...
	 * @param field
	 * @param value
	 * @return If the field already exists, 0 is returned, otherwise if a new field
	 *         is created 1 is returned.
	 */
	@Override
	...
	/**
	 * Increment the number stored at field in the hash at key by a double precision
	 * floating point value. If key does not exist, a new key holding a hash is
	 * created. If field does not exist or holds a string, the value is set to 0
	 * before applying the operation. Since the value argument is signed you can use
	 * this command to perform both increments and decrements.
	 * <p>
	 * The range of values supported by HINCRBYFLOAT is limited to double precision
	 ...
	 * @param field
	 * @param value
	 * @return Double precision floating point reply The new value at field after
	...
	/**
	 * Test for existence of a specified field in a hash. <b>Time complexity:</b>
	 * O(1)
	 * 
	 * @param key
	 * @param field
	...
	/**
	 * Return the length of the list stored at the specified key. If the key does
	 * not exist zero is returned (the same behaviour as for empty lists). If the
	 * value stored at key is not a list an error is returned.
	 * <p>
	 * Time complexity: O(1)
	 * 
	 * @param key
	 ...
	 * index is out of range a 'nil' reply is returned.
	 * <p>
	 ...
	 * to access elements starting from the end of the list. So -1 is the last
	 * element, -2 is the penultimate, and so forth.
	 * <p>
	 * <b>Time complexity:</b>
	 * <p>
	 ...
	 * count is zero all the elements are removed. If count is negative elements are
	 * removed from tail to head, instead to go from head to tail that is the normal
	 * behaviour. So for example LREM with count -2 and hello as value to remove
	 * against the list (a,b,c,hello,x,hello,hello) will lave the list
	 * (a,b,c,hello,x). The number of removed elements is returned as an integer,
	 * see below for more information about the returned value. Note that non
	 * existing keys are considered like empty lists by LREM, so LREM against non
	 ...
	 * @param count
	 * @param value
	 * @return Integer Reply, specifically: The number of removed elements if the
	 *         operation succeeded
	 */
	@Override
	 ...
	 * contains the elements "foo","bar" after an RPOPLPUSH command the content of
	 * the two lists will be "a","b" and "c","foo","bar".
	 * <p>
	 * If the key does not exist or the list is already empty the special value
	 ...
	 * <p>
	 * The {@link #srandmember(String)} command does a similar work but the returned
	 ...
	 * hold at the specified keys. Like in {@link #lrange(String, long, long)
	 ...
	 * {@link #smembers(String) SMEMBERS}. Actually SMEMBERS is just syntax sugar
	 * for SINTER.
	 * <p>
	...
	/**
	 * This commnad works exactly like {@link #sinter(String...) SINTER} but instead
	 ...
	 * the specified keys. Like in {@link #lrange(String, long, long) LRANGE} the
	 ...
	 * this command produces the same result as {@link #smembers(String) SMEMBERS}.
	 * <p>
	...
	/**
	 * This command works exactly like {@link #sunion(String...) SUNION} but instead
	...
	/**
	 * This command works exactly like {@link #sdiff(String...) SDIFF} but instead
	 ...
	 * @param key
	 * @param min
	 *            a double or Double.NEGATIVE_INFINITY for "-inf"
	 * @param max
	 *            a double or Double.POSITIVE_INFINITY for "+inf"
	 * @return Multi bulk reply specifically a list of elements in the specified
	 *         score range.
	 */
	@Override
	 ...
	 * <p>
	 * As the terms imply, the {@link #zinterstore(String, String...) ZINTERSTORE}
	 * command requires an element to be present in each of the given inputs to be
	 * inserted in the result. The {@link #zunionstore(String, String...)
	...
	/**
	 * Undo a {@link #expire(String, int) expire} at turning the expire key into a
	 * normal key.
	 * <p>
	 * Time complexity: O(1)
	 * 
	 ...
	 * issuing a {@link #configGet(String) CONFIG GET *} command.
	 * <p>
	...
	/**
	 * PSETEX works exactly like {@link #setex(String, int, String)} with the sole
