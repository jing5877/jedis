...
public class BinaryShardedJedis extends Sharded<Jedis, JedisShardInfo> implements BinaryJedisCommands {
		...
		for (Jedis jedis : getAllShards()) {
			if (jedis.isConnected()) {
				try {
					...
					// ignore the exception node, so that all other normal nodes can release all
					// connections.
				}
				try {
					jedis.disconnect();
	...
	public List<String> kpop(byte[] key, int partition, int offset) {
		Jedis j = getShard(key);
		return j.kpop(key, partition, offset);
	}

	@Override
	public List<String> kpop(byte[] key, int partition, int offset, int max) {
		Jedis j = getShard(key);
		return j.kpop(key, partition, offset, max);
	}
